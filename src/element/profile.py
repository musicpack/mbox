...
class Profile:
    ...
    def __init__(self, guild: discord.Guild, client: discord.Client, command_channel: discord.TextChannel = None) -> None:
        ...
        self.messenger: Messenger = Messenger(
            guild.voice_channels, FFMPEG_PATH, guild.text_channels[0], client, self.valid_channels)

    async def setup(self):
        if type(self.valid_channels) == discord.TextChannel:
            # Update topic if out of date or malformed
            expected_topic = 'Music Box controlled channel. Chat in this channel will be deleted. Version ' + \
                VERSION + ' ' + str(hash(self.valid_channels))
            if self.valid_channels.topic != expected_topic:
                await self.valid_channels.edit(topic=expected_topic)
            ...
            logging.debug(
                'Guild [{}] is not set up. Sending request to set up.'.format(self.guild))
            ...
            async def action_success(text_channel):
                ...
                topic = 'Music Box controlled channel. Chat in this channel will be deleted. Version ' + \
                    VERSION + ' ' + str(hash(music_box))
            ...
            logging.debug(
                'Guild [{}] has too many valid channels. Sending request to fix.'.format(self.guild))

            offending_channels = []
            for channel in self.valid_channels:
                offending_channels.append(channel.name)

            err_msg = '⚠️Need to remove topic of channels: ' + \
                str(offending_channels)
