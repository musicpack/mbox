...
class PlayerEmbed(Embed):
    def __init__(self, **kwargs) -> None:
        ...
        # Player Variables
        self.display_description: str = kwargs.get('display_description', False)
        ...
        if self.video_title:
            ...
            self.set_author(name=self.video_uploader, url=self.video_uploader_url)
    ...
    def update_footer_text(self, paused: bool = None,volume: int = None, playhead: timedelta = None, queue: Queue = None):
        """Generates new text and updates the footer text in Embed. Generates paused, volume, timeline states."""
        footer_text = self.generate_footer_text(paused=paused, volume= volume, playhead= playhead, queue= queue)
