...
class EventWatcher:
    ...
    async def watch_button_click(
        ...
        if emoji in self.coro:
            logging.warn(
                f"button_click: {emoji} event is already registered. Overwriting event and registering again."
            )
        ...
        async def refresh():
            ...
            try:
                interaction: Interaction = await self.client.wait_for(
                    "button_click", timeout=timeout, check=check
                )
