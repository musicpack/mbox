...
class MusicQueue (ChatEmbed):
    ...
    def __init__(self, client: discord.Client, text_channel: discord.TextChannel, playlist: List[MusicSource] = None) -> None:
        ...
        super().__init__(name='MusicQueue', embed_dict=embed,
                         text_channel=text_channel, actions=[])
    ...
    async def update_embed_from_queue(self) -> None:
        ...
        for index in range(self.index, len(self.playlist)):
            if self.index == index:
                title = 'Queue'
                description_np += '\n> [' + self.playlist[index].info['title'] + \
                    '](' + self.playlist[index].info['webpage_url'] + ')'
            else:
                description_n += '\n> [' + self.playlist[index].info['title'] + \
                    '](' + self.playlist[index].info['webpage_url'] + ')'
        ...
        if self.playlist:
            if self.at_beginning or self.index == None:
                ...
                asyncio.run_coroutine_threadsafe(
                    self.update_embed_from_queue(), self.client.loop)
                return self.playlist[self.index]
