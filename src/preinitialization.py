...
async def fix_duplicate_command_channels(guild: Guild, channels: List[TextChannel]) -> TextChannel:
    ...
    for channel in channels:
        await channel.edit(topic='Old music-box command channel. Safe to delete.')
...
async def clean_chat(text_channel: TextChannel) -> None:
    ...
    async for message in text_channel.history(limit=101):
        if (datetime.today() - message.created_at).days > 14:
            raise ClientException('Text channel contains messages older then 14 days.')
...
async def generate_profiles(guilds: List[Guild], client: Client, profiles: List[Profile]) -> None:
    for server in guilds:
        ...
        if len(channel_list) == 1:
            ...
            validated_channel = await fix_duplicate_command_channels(server, channels= channel_list)
        else:
