...
def gen_config() -> configparser.ConfigParser:
    ...
    generated_config["Cache"]["DOWNLOAD_PATH"] = os.path.join(
        "cache", "youtube"
    )
...
def set_token(config: configparser.ConfigParser):
    ...
    if config_token:
        ...
        return input(
            "No token in config file or in enviroment variable 'DiscordToken_mbox'. Please generate a token and enter it below."
        )
